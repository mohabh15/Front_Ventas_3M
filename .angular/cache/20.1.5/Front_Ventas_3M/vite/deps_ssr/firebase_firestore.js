...
var SimpleDb = class _SimpleDb {
  ...
  static delete(name3) {
    ...
    return wrapRequest(globals.indexedDB.deleteDatabase(name3)).toPromise();
  }
...
function deepClone(source) {
  if (source.geoPointValue) {
    return { geoPointValue: __spreadValues({}, source.geoPointValue) };
  } else if (source.timestampValue && typeof source.timestampValue === "object") {
    return { timestampValue: __spreadValues({}, source.timestampValue) };
...
function fromFound(serializer, doc3) {
  ...
  const version3 = fromVersion(doc3.found.updateTime);
...
function fromDbRemoteDocument(localSerializer, remoteDoc) {
  ...
  if (remoteDoc.document) {
    ...
    doc3 = MutableDocument.newNoDocument(key, version3);
    ...
    doc3 = MutableDocument.newUnknownDocument(key, version3);
  } else {
    return fail(56709);
  }
...
function toDbRemoteDocument(localSerializer, document2) {
  ...
  if (document2.isFoundDocument()) {
    ...
    return fail(57904, { document: document2 });
  }
  return remoteDoc;
}
...
var IndexedDbIndexManager = class {
  ...
  addIndexEntry(transaction, document2, fieldIndex, indexEntry) {
    const indexEntries = indexEntriesStore(transaction);
    return indexEntries.put(indexEntry.dbIndexEntry(this.uid, this.encodeDirectionalKey(fieldIndex, document2.key), document2.key));
  }
  deleteIndexEntry(transaction, document2, fieldIndex, indexEntry) {
    const indexEntries = indexEntriesStore(transaction);
    return indexEntries.delete(indexEntry.dbIndexEntryKey(this.uid, this.encodeDirectionalKey(fieldIndex, document2.key), document2.key));
  }
...
var LocalDocumentsView = class {
  ...
  computeViews(transaction, docs, overlays, existenceStateChanged) {
    ...
    return this.recalculateAndSaveOverlays(transaction, recalculateDocuments).next((recalculatedFields) => {
      ...
      docs.forEach((documentKey, document2) => results.set(documentKey, new OverlayedDocument(document2, mutatedFields.get(documentKey) ?? null)));
      return results;
    });
  }
...
function localStoreSaveNamedQuery(_0, _1) {
  return __async(this, arguments, function* (localStore, query2, documents = documentKeySet()) {
    const allocated = yield localStoreAllocateTarget(localStore, queryToTarget(fromBundledQuery(query2.bundledQuery)));
    const localStoreImpl = debugCast(localStore);
...
var GrpcConnection = class {
  ...
  invokeRPC(rpcName, path, request, authToken, appCheckToken) {
    ...
    const jsonRequest = __spreadValues({ database: this.databasePath }, request);
  ...
  invokeStreamingRPC(rpcName, path, request, authToken, appCheckToken, expectedResponseCount) {
    ...
    const stub = this.ensureActiveStub();
    const metadata = createMetadata(this.databasePath, authToken, appCheckToken, this.databaseInfo.appId);
    const jsonRequest = __spreadProps(__spreadValues({}, request), { database: this.databasePath });
...
function invokeRunAggregationQueryRpc(datastore, query2, aggregates) {
  return __async(this, null, function* () {
    const datastoreImpl = debugCast(datastore);
...
var BundleLoader = class {
  ...
  addSizedElement(element) {
    ...
    if (documentsLoaded !== this.progress.documentsLoaded) {
      ...
      return __spreadValues({}, this.progress);
    }
    return null;
  }
...
function syncEngineSynchronizeWithChangedDocuments(syncEngine, collectionGroup2) {
  return __async(this, null, function* () {
    const syncEngineImpl = debugCast(syncEngine);
  ...
  return __async(this, null, function* () {
    ...
    if (query2 && query2.length > 0) {
      switch (state) {
        case "current":
        case "not-current": {
          const changes = yield localStoreGetNewDocumentChanges(syncEngineImpl.localStore, queryCollectionGroup(query2[0]));
...
function connectFirestoreEmulator(firestore, host, port, options = {}) {
  ...
  const existingConfig = __spreadProps(__spreadValues({}, settings), {
  ...
  const newConfig = __spreadProps(__spreadValues({}, settings), {
    host: newHostSetting,
    ssl: useSsl,
...
function configureFirestore(firestore) {
  ...
  const databaseInfo = makeDatabaseInfo(firestore._databaseId, firestore._app?.options.appId || "", firestore._persistenceKey, settings);
...
function namedQuery(firestore, name3) {
  firestore = cast(firestore, Firestore);
  const client = ensureFirestoreConfigured(firestore);
  return firestoreClientGetNamedQuery(client, name3).then((namedQuery2) => {
    if (!namedQuery2) {
      return null;
    }
...
function registerFirestore(variant, useFetchStreams = true) {
  ...
  _registerComponent(new Component("firestore", (container, { instanceIdentifier: databaseId, options: settings }) => {
    ...
    settings = __spreadValues({ useFetchStreams }, settings);
...
var ParseContextImpl = class _ParseContextImpl {
  ...
  contextWith(configuration) {
    return new _ParseContextImpl(__spreadValues(__spreadValues({}, this.settings), configuration), this.databaseId, this.serializer, this.ignoreUndefinedProperties, this.fieldTransforms, this.fieldMask);
  }
...
function documentToDocumentSnapshotBundleData(path, documentData, document2) {
  return {
    documentData,
...
function documentSnapshotFromJSON(db, json, converter) {
  if (validateJSON(json, DocumentSnapshot._jsonSchema)) {
    ...
    const document2 = fromDocument(serializer, bundledDocuments[0].document);
    const documentKey = new DocumentKey(ResourcePath.fromString(json.bundleName));
    return new DocumentSnapshot(db, new LiteUserDataWriter(db), documentKey, document2, new SnapshotMetadata(
      /* hasPendingWrites= */
      false,
      /* fromCache= */
      false
...
function querySnapshotFromJSON(db, json, converter) {
  if (validateJSON(json, QuerySnapshot._jsonSchema)) {
    ...
    bundledDocuments.map((bundledDocument) => {
      const document2 = fromDocument(serializer, bundledDocument.document);
      documentSet = documentSet.add(document2);
    });
...
function onSnapshotQuerySnapshotBundle(db, json, options, observer, converter) {
  ...
  loadTask.then(() => namedQuery(db, json.bundleName)).then((query2) => {
    if (query2 && !unsubscribed) {
      ...
      internalUnsubscribe = onSnapshot(query2, options ? options : {}, observer);
    }
  }).catch((e) => {
    if (observer.error) {
      observer.error(e);
...
function runTransaction(firestore, updateFunction, options) {
  firestore = cast(firestore, Firestore);
  const optionsWithDefaults = __spreadValues(__spreadValues({}, DEFAULT_TRANSACTION_OPTIONS), options);
